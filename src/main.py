...
import argparse, os, re, time, multiprocessing as mp
...
def save_fail_screenshot(driver, url: str, reason: str) -> str:
    if not SCREENSHOT_ON_FAIL: return ""
    ...
    try: driver.save_screenshot(path); return path
...
def cmd_analyze(args: argparse.Namespace) -> None:
    ...
    for i, row in df.iterrows():
        ...
        if ok: df.at[i, "status"] = "Analyzed"; df.at[i, "notes"] = ""
        else:  df.at[i, "status"] = "Skipped";  df.at[i, "notes"] = reason
    save_rows(df, out); logger.info(f"Analyze done -> {out}")
...
def cmd_scan(args: argparse.Namespace) -> None:
    ...
    try:
        for pos, (i,row) in enumerate(rows, 1):
            ...
            if not url: continue
            if any(_row_to_selectors(row).values()):  # đã có template trong excel
                continue
            sel = lookup(cache, url) or discover_form(driver, url)
            if sel:
                upsert(cache, url, sel, scope=scope); scanned += 1
                if write_template: _write_selectors_to_row(df, i, sel, scope=scope)
            if scanned % save_every == 0:
                save_cache(cache, cache_path)
                if write_template: save_rows(df, inp)
            if pos % BATCH_SIZE == 0:
                try: driver.quit()
                except: pass
                driver = build_driver(HEADLESS)
...
def cmd_post(args: argparse.Namespace) -> None:
    ...
    if start > 0: rows_all = rows_all[start:]
    if limit > 0: rows_all = rows_all[:limit]
    ...
    for i,row in rows_all:
        ...
        tpl = _row_to_selectors(row); has_tpl = any(tpl.values())
        if use_template_only and not has_tpl:
            df.at[i,"status"]="Failed"; df.at[i,"notes"]="No template selectors"; continue
    ...
    if dry_run:
        for i, *_ in tasks: df.at[i,"notes"]="DRY RUN: would open and post"
        save_rows(df, inp); logger.info(f"DRY RUN updated -> {inp}"); return

    if workers == 1:
        res = _worker_post(tasks, {"headless": HEADLESS, "pause_min": PAUSE_MIN, "pause_max": PAUSE_MAX})
        for i,st,note in res: df.at[i,"status"]=st; df.at[i,"notes"]=note
    else:
        ...
        with mp.get_context("spawn").Pool(processes=workers) as pool:
            for res in pool.imap_unordered(lambda c: _worker_post(c, options), chunks):
                for i,st,note in res: df.at[i,"status"]=st; df.at[i,"notes"]=note
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    cmd_analyze(a)
    if args.cache:
        s = argparse.Namespace(input=args.output or OUTPUT_XLSX, scope="domain", cache=args.cache, start=0, limit=0, save_every=20, write_template=True)
        cmd_scan(s)
    p = argparse.Namespace(input=args.output or OUTPUT_XLSX, start=0, limit=0, save_every=5, dry_run=False, cache=args.cache, prefer_template=True, use_template_only=False, workers=getattr(args,"workers",1), chunk=80)
    cmd_post(p)

def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="blog-comment-tool", description="Fast SEO blog comment tool (pre-filter + template + parallel)")
    sub = p.add_subparsers(dest="command", required=True)

    pa = sub.add_parser("analyze"); pa.add_argument("--input","-i",default=INPUT_XLSX); pa.add_argument("--output","-o",default=OUTPUT_XLSX); pa.set_defaults(func=cmd_analyze)
    ps = sub.add_parser("scan");    ps.add_argument("--input","-i",default=INPUT_XLSX); ps.add_argument("--cache",default="data/forms_cache.json"); ps.add_argument("--scope",choices=["domain","path"],default="domain"); ps.add_argument("--start",type=int,default=0); ps.add_argument("--limit",type=int,default=0); ps.add_argument("--save-every",type=int,default=20); ps.add_argument("--write-template",action="store_true"); ps.set_defaults(func=cmd_scan)
    pp = sub.add_parser("post");    pp.add_argument("--input","-i",default=OUTPUT_XLSX); pp.add_argument("--start",type=int,default=0); pp.add_argument("--limit",type=int,default=0); pp.add_argument("--save-every",type=int,default=5); pp.add_argument("--dry-run",action="store_true"); pp.add_argument("--cache",default="data/forms_cache.json"); pp.add_argument("--prefer-template",action="store_true"); pp.add_argument("--use-template-only",action="store_true"); pp.add_argument("--workers",type=int,default=1); pp.add_argument("--chunk",type=int,default=60); pp.set_defaults(func=cmd_post)
    pr = sub.add_parser("run");     pr.add_argument("--input","-i",default=INPUT_XLSX); pr.add_argument("--output","-o",default=OUTPUT_XLSX); pr.add_argument("--cache",default="data/forms_cache.json"); pr.add_argument("--workers",type=int,default=1); pr.set_defaults(func=cmd_run)
    return p
